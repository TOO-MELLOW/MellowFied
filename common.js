...
/**
 * MellowTech Conversational Assistant Engine v2.0
 * ================================================
 * Architecture: NLP Pipeline → Intent Classifier → State Machine → Response Renderer
 *
 * Key capabilities (rule-based, zero dependencies):
 *  ① Text normalization  — typo correction, Afrikaans slang, SMS abbreviations
 *  ② Weighted TF scoring — multi-signal intent classification with confidence
 *  ③ Disambiguation      — detects ambiguous input, asks clarifying questions
 *  ④ Context memory      — remembers conversation turns, resolves short follow-ups
 *  ⑤ Entity extraction   — detects deadlines ("due tomorrow"), urgency signals
 *  ⑥ Sentiment detection — frustrated users get escalation path faster
 *  ⑦ Multi-turn flows    — e.g. assignment + deadline = tailored response
 *  ⑧ Rich response format— lists, scenarios, pricing, turnaround, follow-up Qs
 *  ⑨ Self-contained CSS  — styles injected at runtime, no extra stylesheet needed
 */
...
;(function (root) {
  ...
  chips: '#mtChips'
    ...
    function showTyping() {
      const box = $(SEL.chatWindow);
      if (!box) return null;
      const el = document.createElement("div");
      el.className = "mwt-msg mwt-bot mwt-typing";
      el.innerHTML = `<div class="mwt-bubble"><span></span><span></span><span></span></div>`;
      box.appendChild(el);
    ...
    function appendMsg(role, html) {
      ...
      wrap.className = `mwt-msg mwt-${role}`;
      wrap.innerHTML = `<div class="mwt-bubble">${html}</div>`;
    ...
    function renderSuggestions(suggestions) {
      ...
      const box = $(SEL.chatWindow);
      if (!box) return;
      // Remove previous suggestion rows to prevent clutter
      box.querySelectorAll(".mwt-suggestions").forEach(el => el.remove());

      const row = document.createElement("div");
      row.className = "mwt-suggestions";
    ...
    function handleInput(text) {
      ...
      // Delay scales with input length for natural feel (min 600ms, max ~1.5s)
      const delay = Math.min(600 + text.length * 10, 1500);

      setTimeout(() => {
        if (typing) typing.remove();
        const { html, suggestions } = process(text);
        appendMsg("bot", html);
    ...
    function sendWelcome() {
      const box = $(SEL.chatWindow);
      if (!box || box.dataset.mwtReady) return;
      box.dataset.mwtReady = "1";
      setTimeout(() => {
        appendMsg("bot",
          `<p>👋 Hi! I'm the <strong>MellowTech Assistant</strong>.</p>
           <p>Ask me anything about our services, pricing, turnaround times, how we work — or just tell me what you need.</p>`
        );
        renderSuggestions(["🛠️ Services", "💰 Pricing", "📄 CV Help", "🌐 Website", "⏱ Turnaround", "📞 Contact"]);
      }, 450);
    }
    ...
    function init() {
      ...
      document.querySelectorAll(
        ".quick-replies button, .quick-btns button, .chat-quick-replies button"
      ).forEach(btn => {
      ...
      if (toggle) {
        ...
        toggle.addEventListener("click", () => {
          if (!opened) { opened = true; sendWelcome(); }
        });
      } else {
        sendWelcome();
      }
